...
class MovingTargetDefense(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.host_vips: DefaultDict[str, Set[str]] = defaultdict(set)
    ...
    def _bind_primary_vip(self, host_ip: str, vip: str, now: float):
        ...
        self._log_vip_table("bind")

    def _log_vip_table(self, reason: str):
        rows = []
        for host_ip in sorted(self.detected_hosts):
            primary = self.primary_vip.get(host_ip, "-")
            all_vips = ",".join(sorted(self.host_vips.get(host_ip, set()))) or "-"
            rows.append(f"{host_ip}: primary={primary} all=[{all_vips}]")
        if rows:
            self.logger.info("VIP TABLE (%s): %s", reason, " | ".join(rows))
        else:
            self.logger.info("VIP TABLE (%s): <empty>", reason)
    ...
    def _add_flow(self, dp, priority, match, actions, cookie=0, table_id=0,
        ...
        # Be explicit with all OpenFlow flow-mod scalar fields to avoid
        # cross-version parser defaults that can become `None` in some Ryu
        # releases and trigger serialization errors.
    ...
    def _delete_flow(self, dp, match=None, cookie=0, cookie_mask=0, table_id=None):
        parser = dp.ofproto_parser
        ofp = dp.ofproto
        if table_id is None:
    ...
    def _packet_in(self, ev):
        ...
        if vip_dst:
            ...
            vip_src_log = self.primary_vip.get(src_real, src_ip)
            self.logger.info("VIP TRANSLATION: %s(%s) -> %s(%s) [wire-dst=%s]",
                             src_real, vip_src_log, real_dst, vip_dst, real_dst)
