...
class MovingTargetDefense(app_manager.RyuApp):
    ...
    def _packet_in(self, ev):
        ...
        # For real-IP destinations, keep the existing VIP binding as long as the
        # session remains active. VIP rotations only affect new sessions; active
        # flows retain their established vip_dst.
        if direction == 'forward' and not vip_dst:
            vip_owner_matches = session.vip_dst and self.V2R_Mappings.get(session.vip_dst) == server_real
            if vip_owner_matches:
                vip_dst = session.vip_dst
            else:
                if session.vip_dst and session.vip_dst in self.V2R_Mappings:
                    released = self._detach_session_from_vip(session.vip_dst, session_key)
                    if released:
                        owner = self.V2R_Mappings.get(session.vip_dst)
                        if owner:
                            self._evaluate_host_state(owner, now)
                session.vip_dst = None
                session.last_reply_vip = None
                session.reverse_flow_cookie = None

            if not vip_dst:
                vip_dst = self._ensure_primary_vip(server_real, now, force=True)

            if vip_dst:
                if session.vip_dst != vip_dst and session.vip_dst:
                    released = self._detach_session_from_vip(session.vip_dst, session_key)
                    if released:
                        owner = self.V2R_Mappings.get(session.vip_dst)
                        if owner:
                ...
                if proto != 1 and not session.vip_locked:
                    session.vip_locked = vip_dst
                self._touch_vip(vip_dst, now, "session create: vip_dst (real dst)")
                if new_session:
                    self._activate_vip_for_session(vip_dst, session_key, now)
                self._register_reply_mapping(session, server_real, client_real,
                                             proto, vip_dst, client_port, server_port,
