...
# -*- coding: utf-8 -*- 
# =============================================================================
# industry_topo.py — Industry-realistic MTD topology
...
#
# Usage:
#   sudo python industry_topo.py 4     # 4-port  switches (~16  hosts)
#   sudo python industry_topo.py 24    # 24-port switches (~108 hosts)
#   sudo python industry_topo.py 48    # 48-port switches (~504 hosts)
# =============================================================================
...
def run(port_count):
    ...
    # Mininet/ryu topology API (get_link) reports switch-to-switch links only.
    # It does NOT include host-edge links.
    # Physical switch links = agg-core + edge-agg
    # directed_links = physical_switch_links * 2
    physical_switch_links = topo.n_agg + topo.n_edge
    expected_directed_links = physical_switch_links * 2
    topo_expect = {
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        print('Usage: sudo python industry_topo.py <port_count>')
        print('')
        print('  port_count : 4, 24, or 48')
        print('')
        print('Examples:')
        print('  sudo python industry_topo.py 4    # 4-port  switches (~16  hosts)')
        print('  sudo python industry_topo.py 24   # 24-port switches (~108 hosts)')
        print('  sudo python industry_topo.py 48   # 48-port switches (~504 hosts)')
        sys.exit(1)
